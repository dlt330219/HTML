...
TiXmlElement *search_element_by_attribute(TiXmlElement *element, char *attribute_name, char *attribute_value )
{
    TiXmlNode *child;
...
void write_fields_into_xml(Memory_mgt *memory_manager)
{
	...
	memory_manager->get_fields_mem(fields_mem);
    	...
    	{
		char *comp_id;
		sprintf(comp_id, "%d", fields_mem[i]->get_comp_id());
		TiXmlElement *root_or_comp_element = search_element_by_attribute(root_element,"comp_id",comp_id);
		TiXmlElement *field_parent_element = root_or_comp_element;
		if (root_or_comp_element == root_element)
		{
